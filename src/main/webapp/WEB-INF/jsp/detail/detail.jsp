    ...
    <%@ taglib uri="jakarta.tags.core" prefix="c" %>
        <%@ taglib uri="jakarta.tags.functions" prefix="fn" %>
            <%@ include file="/WEB-INF/jsp/fragments/header.jsp" %>
				...
				<c:choose>
				  <c:when test="${not empty restaurant.image and (fn:startsWith(restaurant.image,'http://') or fn:startsWith(restaurant.image,'https://'))}">
				    <img class="detail-img"
				         src="${restaurant.image}"
				         alt="${restaurant.name}"
				         loading="lazy"
				         onerror="this.onerror=null;this.src='${ctx}/img/no-image.png';" />
				  </c:when>

				  <c:when test="${not empty restaurant.image and fn:startsWith(restaurant.image,'/')}">
				    <img class="detail-img"
				         src="${restaurant.image}"
				         alt="${restaurant.name}"
				         loading="lazy"
				  ...
				  <c:when test="${not empty restaurant.image}">
				    <img class="detail-img"
				         src="${ctx}/uploads/${restaurant.image}"
				         alt="${restaurant.name}"
				         loading="lazy"
				         onerror="this.onerror=null;this.src='${ctx}/img/no-image.png';" />
				  </c:when>
				  ...
				  <c:otherwise>
				    <img class="detail-img"
                        ...
                        <div class="detail-right">
                            <div class="reservation-box">
                                ...
                                <!-- ✅ 예약 입력 -->
                                <form id="reserveForm" action="${pageContext.request.contextPath}/api/reservations"
                                    ...
                                    <input id="phoneInput" type="text" name="customerPhone" placeholder="연락처"
                                        required />
                                    <input id="partyInput" type="number" name="partySize" placeholder="인원수" min="1"
                                        value="2" required style="width:90px" />
                    ...
                    <div class="review-section">
                        ...
                        <form class="review-form"
                            action="${pageContext.request.contextPath}/restaurants/${restaurant.id}/reviews"
                            method="post">
                    ...
                    <%@ include file="/WEB-INF/jsp/fragments/footer.jsp" %>
                        ...
                        if(!window.IS_LOGGED_IN){ ev.preventDefault();
                        location.href="${pageContext.request.contextPath}/login.jsp?next="+encodeURIComponent(location.pathname+location.search);
                        return; }
